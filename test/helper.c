...
{
    char *output = gotoURL(path);
    char **folders;

    int numFolders = -1;
    int outputLen = strlen(output);

    for (int i = 0; i < outputLen; i++) if (output[i] == '\n') numFolders++;

    char folder[1000];

    folders = (char**) malloc(sizeof(char*) * (numFolders + 1));
    folders[numFolders] = 0;

    int index = 0;

    FILE* bigStream = fmemopen(output, outputLen, "r");
    fgets(folder, 1000, bigStream);
    while(fgets(folder, 1000, bigStream))
    {
        FILE* smallStream = fmemopen(folder, strlen(folder), "r");
        char temp[1000];
        for (int i = 0; i < 5; i++)
        {
            fscanf(smallStream, "%s", temp);
            temp[strlen(temp)-1] = 0;
        }
        char *folder_name, *parent_dir;
        extractFileDetails(temp, &folder_name, &parent_dir);
        free(parent_dir);
